...
const GameScreen = (props: Props) => {
  ...
  const [state,setState] = useState<string>("Le jeu commence quand tous les joueurs sont prêts  ✅");
...
const ItemAnswer = (id:string,answer:string) =>{
  ...
  return (
    <div className="containerGame">
      ...
      <Modal visible={isVisible} cancelText="Non" okText="Valider" onCancel={()=>{
        setIsVisible(false);
        setError(false)
        }} onOk={()=>setTeamName()}>
      <h1>Voulez-vous être dans une équipe?</h1>
      <input className="inputTeam" placeholder="Entrer un nom d'équipe" type="text" id="inputTeam" />
      </Modal>
...
<Carousel>
     ...
     { users.forEach((user:any) => 
      ...
      {dataJeu.type === 3 && 
        (<TextComponent text1={dataJeu.text1} answer={user.answer} text2={dataJeu.text2} />)
        }
        <Carousel.Caption>
          {dataJeu.type !== 3 && 
          (
